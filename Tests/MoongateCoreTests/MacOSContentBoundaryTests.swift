...
final class MacOSContentBoundaryTests: XCTestCase {
    ...
    func testReadyPageFormatAndSubtitleControlsUseLocalizer() throws {
        ...
        XCTAssertTrue(source.contains("section(localizer.t(L.Ready.sourceLanguageSection))"))
    ...
    func testReadyPageSeparatesPrimarySubtitleSourceFromSubtitleOutput() throws {
        ...
        XCTAssertTrue(readyBody.contains("section(localizer.t(L.Ready.sourceLanguageSection))"))
        XCTAssertTrue(readyBody.contains("sourceLanguageRows(info)"))
        ...
        // 来源区：只保留技术来源策略、导入文件与可行动提示；原声语言不藏进高级区。
    ...
    func testPrimarySubtitleSourceRowsExposeManualAndAutoGeneratedAccessibilitySemantics() throws {
        ...
        XCTAssertTrue(badgeBody.contains("localizer.t(L.Ready.cloudASR)"))
