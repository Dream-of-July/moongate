...
final class MacOSContentBoundaryTests: XCTestCase {
    ...
    func testReadyPageFormatAndSubtitleControlsUseLocalizer() throws {
        ...
        XCTAssertTrue(source.contains("section(localizer.t(L.Ready.subtitleLanguageSection))"))
    ...
    func testReadyPageShowsLocalASRSubtitleSourceAsSelectableTrack() throws {
        ...
        XCTAssertTrue(readyBody.contains("subtitleLanguageRows(info)"))
        XCTAssertTrue(readyBody.contains("sourceLanguagePreferencePicker(info)"))
    ...
    func testReadyPageSeparatesPrimarySubtitleSourceFromSubtitleOutput() throws {
        ...
        // 语言选择区与字幕输出区分离的两个 section。
        XCTAssertTrue(readyBody.contains("section(localizer.t(L.Ready.subtitleLanguageSection))"))
        XCTAssertTrue(readyBody.contains("sourceLanguagePreferencePicker(info)"))
        XCTAssertTrue(readyBody.contains("subtitleLanguageRows(info)"))
        ...
        // 主区域：推荐语言 + 自动定源说明 + “不需要字幕”；展开区：更多语言。
        XCTAssertTrue(source.contains("localizer.t(L.Ready.recommendedBadge)"))
        XCTAssertTrue(rowsBody.contains("localizer.t(L.Ready.autoSourceExplanation)"))
        XCTAssertTrue(rowsBody.contains("localizer.t(L.Ready.noSubtitleSource)"))
        XCTAssertTrue(rowsBody.contains("model.primarySubtitleTrackID == nil"))
